...
public class QuanLyLoaiSanPhamForm extends JPanel {
    ...
    // index
    final int MALSP_I = 1, TENLSP_I = 2;

    public QuanLyLoaiSanPhamForm() {
        mtb = new MyTable();
        mtb.setPreferredSize(new Dimension(1200 - 250, 600));
        mtb.setHeaders(new String[]{"STT", "Mã loại", "Tên loại"});
        mtb.setColumnsWidth(new double[]{.5, 4, 5});
        ...
        // ======== search panel ===========
        cbTypeSearch = new JComboBox<>(new String[]{"Tất cả", "Mã loại", "Tên loại"});
    ...
    private void btnXoaMouseClicked() {
        ...
        if (i >= 0) {
            String malsp = mtb.getModel().getValueAt(i, MALSP_I).toString();
            if (JOptionPane.showConfirmDialog(null, "Bạn có chắc muốn xóa sản phẩm " + malsp) == JOptionPane.OK_OPTION) {
                qllsp.delete(malsp);
//                mtb.getModel().removeRow(i);
    ...
    private void setDataToTable(ArrayList<LoaiSanPham> data, MyTable table) {
        ...
        for (LoaiSanPham lsp : data) {
            table.addRow(new String[]{String.valueOf(stt), lsp.getMaLSP(), lsp.getTenLSP()});
            stt++;
        }
    }
